...
namespace game {
    using bitops4::Bitboard;
    using bitops4::FULL;
    using bitops4::NOT_A;
    using bitops4::NOT_D;
    using bitops4::vertical4;
    using bitops4::horizontal4;
    using bitops4::transpose4;
    using bitops4::N;
    ...
    inline constexpr std::uint8_t MAX_TIER = 16;
    ...
    // For 4x4, 3 cascades are sufficient across any line.
    ...
    inline Encoding do_move(const Encoding& pos, Bitboard mv) {
        ...
        const Bitboard flips =
            ...
            sweep_r(mv, me, opp, 1,    NOT_D) |  // West
            sweep_l(mv, me, opp, N,    FULL ) |  // North
            sweep_r(mv, me, opp, N,    FULL ) |  // South
            sweep_l(mv, me, opp, N+1,  NOT_A) |  // NE
            sweep_r(mv, me, opp, N+1,  NOT_D) |  // SW
            sweep_l(mv, me, opp, N-1,  NOT_D) |  // NW
    ...
    inline Encoding canonical(const Encoding& pos) noexcept {
        ...
        const Bitboard occ1 = vertical4(occ);        // V
        const Bitboard occ2 = horizontal4(occ);      // H
        const Bitboard occ3 = transpose4(occ);       // T
        const Bitboard occ5 = vertical4(occ3);       // R90 = V∘T
        const Bitboard occ7 = horizontal4(occ3);     // R270 = H∘T
        const Bitboard occ6 = horizontal4(occ1);     // R180 = H∘V
        const Bitboard occ4 = horizontal4(occ5);     // ADIAG= H∘R90
        ...
        if (tie & (1u<<1)) {
            me_v = vertical4(me); have_v = true;
            if (me_v < best_me) best_me = me_v;
        }

        // H
        if (tie & (1u<<2)) {
            const Bitboard m2 = horizontal4(me);
        ...
        if (tie & (1u<<3)) {
            me_t = transpose4(me); have_t = true;
            if (me_t < best_me) best_me = me_t;
        }

        // ADIAG = H∘R90 = H∘V∘T
        if (tie & (1u<<4)) {
            if (!have_t) { me_t = transpose4(me); have_t = true; }
            const Bitboard m5tmp = vertical4(me_t);      // R90
            const Bitboard m4 = horizontal4(m5tmp);   // ADIAG
        ...
        if (tie & (1u<<5)) {
            if (!have_t) { me_t = transpose4(me); have_t = true; }
            const Bitboard m5 = vertical4(me_t);
            if (m5 < best_me) best_me = m5;
        }

        // R180 = H∘V
        if (tie & (1u<<6)) {
            if (!have_v) { me_v = vertical4(me); have_v = true; }
            const Bitboard m6 = horizontal4(me_v);
        ...
        if (tie & (1u<<7)) {
            if (!have_t) { me_t = transpose4(me); have_t = true; }
            const Bitboard m7 = horizontal4(me_t);
